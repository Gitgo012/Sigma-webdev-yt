...
function clearAll() {
    ...
    const jokeGenerator=document.querySelector('.joke-content');
    const quoteGenerator=document.querySelector('.quote-content');
    const riddleGenerator=document.querySelector('.riddle-content');
    const answerGenerator=document.querySelector('.reveal-btn');

    if (memeGenerator) memeGenerator.innerHTML = '';
    if (jokeGenerator) jokeGenerator.innerHTML = '';
    if (quoteGenerator) quoteGenerator.innerHTML = '';
    if (riddleGenerator) riddleGenerator.innerHTML = '';
    if (answerGenerator) answerGenerator.innerHTML= '';
}
...
function showRiddle() {
    ...
    currentRiddle=randomRiddle.answer;
    // Get user input after a delay
    setTimeout(() => {
      const userInput = prompt("Enter the answer: ");
  
      // Display user input
      const userInputDisplay = document.createElement('p');
      userInputDisplay.textContent = `Your answer: ${userInput}`;
      riddleContainer.appendChild(userInputDisplay);
  
      // Check the answer
      const answerChecker = document.createElement('p');
      if (userInput === randomRiddle.answer) {
        answerChecker.textContent = "Correct Answer!";
      } else {
        answerChecker.textContent = "Wrong Answer! Press reveal answer to know the correct answer.";
      }
      riddleContainer.appendChild(answerChecker);
...
function revealAnswers() {
    const riddleContainerchecker=document.querySelector('.riddle-content');


    if (riddleContainerchecker.innerHTML.trim()===''){
        alert("There is no riddle");
    }
    const answerContainer = document.querySelector(".reveal-btn");

    // Check if the answer is already displayed to avoid duplicates
    const existingAnswer = document.querySelector('.reveal-btn p');
    if (!existingAnswer) {
        const displayAnswer = document.createElement('p');
        displayAnswer.textContent = currentRiddle;
        answerContainer.appendChild(displayAnswer);
    } else {
